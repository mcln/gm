<x-AppLayout>
    <div class="div-ampliar mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 py-8">  
        ...
        <div class="flex rounded-lg bg-azul-oscuro py-2 px-4 items-center">
            <a href="{{ route('exercises.sector', $sections->first()->chapter->sector->id) }}" class="inline-flex items-center bg-azul-oscuro hover:bg-azul-electrico text-white font-bold py-1 px-3 rounded-full">
        ...
        <div class="rounded-lg grid grid-cols-1 gap-1 py-4 px-4 bg-azul-oscuro">
            ...
            @foreach ($sections as $section)
                <a href="{{ route('exercises.section', ['section' => $section->id]) }}" class="bg-white hover:bg-azul-claro rounded-lg">
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ ucfirst($section->name) }}</span>
                </a>
            @endforeach
        </div>
    </div>
