<div>
    @if (auth()->check())
        <button
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="#0F4069"
                stroke-width="2">
                            ...
                            <form wire:submit.prevent="submit">
                                ...
                                <div class="mb-4">
                                    <label for="description"
                                        class="block text-gray-700 font-bold mb-2">Descripción del
                                        problema:</label>
                ...
                <div class="bg-azul-claro rounded-lg overflow-hidden shadow-xl max-w-md w-full">
                    <div class="px-6 py-4">
                        ...
                        <form class="mt-1" wire:submit.prevent="submit">
                            <div>
                                <label for="description"
                                    class="block text-sm font-medium text-gray-700 mb-2">Descripción del
                                    problema</label>
                                <textarea id="description" name="description_error" rows="1" wire:model.defer="description_error"
                                    class="form-input rounded-md shadow-sm mt-1 text-sm block w-full" required></textarea>
                                @error('description')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="flex flex-col mt-4">
                                <span wire:show="showSuccessMessage && showForm == false"
                                    class="font-bold text-center mb-2 text-green-500">Reporte enviado
                                    satisfactoriamente.</span>
                                <button type="submit"
                                    class="inline-block bg-azul-oscuro hover:bg-azul-electrico text-white font-bold py-2 px-4 rounded-full">
                                    Enviar reporte
                                </button>
                            </div>
                        </form>
